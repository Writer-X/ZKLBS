                           ...
                           int time, int pk, int rand, int n)
{
   typedef libff::Fr<default_r1cs_gg_ppzksnark_pp> FieldT;
    ...
    pb.val(fa) = -110;
    pb.val(sub_1) = coordinate.first - 110;
    pb.val(mux_1) = (coordinate.first - 110) * (coordinate.first - 110);

    pb.val(y) = coordinate.second;
    
    pb.val(fb) = -32;
    pb.val(sub_2) = coordinate.second - 32;
    pb.val(mux_2) = (coordinate.second - 32) * (coordinate.second - 32);

    pb.val(r) = 10;
    pb.val(mux_r) = 10 * 10;  

    pb.val(out) = (coordinate.first - 110) * (coordinate.first - 110) + (coordinate.second - 32) * (coordinate.second - 32);
